...
const FloatingButton = ({
    ...
    return (
        <View className='absolute right-5 bottom-8 z-50'>
            ...
            >
                {type === 'add' && <Ionicons name="add" size={40} color='white' />}
                {type === 'write' && <MaterialIcons name="edit" size={40} color='white' />}
            </TouchableOpacity>
        </View>
    );
};

const FloatingButtonVariant = cva('flex flex-col justify-center items-center rounded-full bg-main p-3 w-[70] h-[70]', {
    variants: {
        platform: {
