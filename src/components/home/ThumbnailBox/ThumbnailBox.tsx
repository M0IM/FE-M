  ...
  return (
    ...
    >
      ...
      <TouchableOpacity className='absolute flex flex-row items-center bottom-4 w-[250] h-[50] ' activeOpacity={0.8}>
        {spaceImg ? (
          <Image source={{uri: spaceImg}} width={40} height={40} className='ml-4 mr-3 rounded-full'/>
        ) : (
        ...
        <View className='w-[160]'>
          <Typography fontWeight='MEDIUM' className='text-white' numberOfLines={1}>{spaceName}</Typography>
        </View>
      </TouchableOpacity>
    </TouchableOpacity>
  );
