...
const SchedulePreviewCard = ({
    ...
    const participantsCount = participants.length > 3 ? participants.slice(0, 3) : participants;
    ...
    return (
        ...
        >
        <Typography fontWeight='BOLD' className='text-base' numberOfLines={1}>{title}</Typography>
        <View className='flex flex-row items-center mt-2'>
            <View className='w-[7px] h-[7px] bg-main rounded-full mr-2' />
            <Typography fontWeight='MEDIUM' className='text-gray-300 text-sm'>{date}</Typography>
        </View>
        <Typography fontWeight='MEDIUM' className='text-gray-300 text-sm' numberOfLines={1}>{place}</Typography>
