...
const PermissionManageScreen = ({route}: PermissionManageScreenProps) => {
  ...
  const { useGetInfinityMoimMembers, updateMoimAuthoritiesMutation } = useMoimManagment();
  const {
    data: moimMembers,
    fetchNextPage,
    hasNextPage,
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View key={item.userId} className='flex flex-row items-center py-3'>
            ...
            <Typography fontWeight='MEDIUM' className='text-dark-800 text-sm ml-3 mr-3'>{item.nickname}</Typography>
            {item.moimRole !== 'OWNER' && <Label label={item.moimRole} color="dark" />}
            <TouchableOpacity className='p-2 rounded-xl bg-gray-200 ml-auto' onPress={() => handleMemberAuth(item.userId, item.moimRole)}>
              <Typography fontWeight='MEDIUM' className='text-gray-600 text-xs'>{item.moimRole === 'ADMIN' ? '권한 취소' : '권한 부여'}</Typography>
            </TouchableOpacity>
          </View>
        )}
