...
export default function TodoCreateScreen({
  ...
  const {imageUri, uploadUri, handleChange, deleteImageUri} =
    useSingleImagePicker({initialImage: todoList?.imageUrlList[0] || ''});
  const [selectedIds, setSelectedIds] = useState<number[]>([]);
  const {modifyTodoMutation} = useTodo();
  ...
  return (
    <ScreenContainer
      fixedBottomComponent={
        <CustomButton
          className={`${modifyTodoMutation.isPending ? 'bg-gray-300' : ''}`}
          textStyle="text-white text-lg font-bold"
          label={`${modifyTodoMutation.isPending ? '배정 중...' : '할 일 배정'}`}
          onPress={handleModifyTodo}
          disabled={modifyTodoMutation.isPending}
        />
      }>
      <View className="mt-4">
